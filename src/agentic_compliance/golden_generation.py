"""Golden test-case generation logic.
...
data/golden_set.yaml, which the evaluation harness consumes. All generated cases
start human_verified: false — this module never marks a case verified; that's a human
judgment call, not something to fake.

`scripts/generate_golden.py` is a thin CLI wrapper over this module (kept as a
standalone script deliberately, not a CLI subcommand — golden generation is dev-time
dataset production, not product surface).
"""
...
def _relevant_controls(fixture_root: Path, retriever: Any, top_k: int) -> list[Any]:
    ...
    from .control_selection import select_controls  # noqa: PLC0415
...
def _strip_comment_lines(content: str) -> str:
    """Strip whole-line '#'-prefixed comments before handing content to the labeler.

    Fixture header comments intentionally describe which verdict/control the fixture
    exercises (e.g. "Intended GAP evidence for AC-3...") — useful for a human reading
    the file, but exactly the kind of text that would hand the labeler model the
    answer instead of letting it reason from evidence (confirmed as a real bias risk
    during evaluation review; the runtime assessment pipeline is unaffected since it
    never reads raw fixture content — only scanner-derived EvidenceRef excerpts).

    A best-effort label-bias reducer, not a security boundary: it strips whole-line
    '#'-prefixed comments only — no inline trailing comments, no string-literal
    awareness, and no handling for block comments (e.g. Terraform's '/* ... */',
    which isn't line-prefixed and would pass through untouched). Adequate for the
    current fixture set, which is small, synthetic, and comments with '#' throughout
    (Python/Terraform/YAML/Dockerfile) — see tests/fixtures/repos/README.md. If a
    future fixture uses block comments to describe an intended verdict, either avoid
    label-revealing prose there or extend this stripper; either way, remember golden
    labels are human-reviewed regardless (docs/DECISIONS.md D8), which is the real
    backstop against a labeler picking up biased text.
    """
...
def _repo_digest(fixture_root: Path) -> str:
    ...
    raw walk, so this module can't reintroduce the symlink-escape class of bug fixed
    in control_selection.py's Terraform resource detection. Comments are stripped
    before truncation so the character budget goes to real content, not header prose.
    """
    from .repo_loader import iter_repo_files, read_file_slice  # noqa: PLC0415
    ...
    for repo_file in repo_files[:_MAX_FILES_PER_FIXTURE]:
        ...
        content = _strip_comment_lines(excerpt.content)[:_MAX_CHARS_PER_FILE]
        block = f"--- {rel} ---\n{content}\n"
